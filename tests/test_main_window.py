...
Settings saves no longer relaunch — every axis (toggles, model/provider,
hotkey) rides ``MurmurApp.reload_config`` in-process. See #38 for the
end-to-end story.
"""
...
def test_hotkey_change_emits_config_saved_without_relaunch(qapp):
    """A hotkey change is now applied in-process by MurmurApp.reload_config
    via the config_saved signal. _persist_changes must emit it and must
    not invoke any restart machinery (the module no longer imports any)."""
    saved: list[config_mod.Config] = []
    emitted: list[config_mod.Config] = []
    win = MainWindow(_make_cfg(), save_config=saved.append)
    win.config_saved.connect(emitted.append)
    ...
    assert emitted and emitted[-1].hotkey == "<f9>"
