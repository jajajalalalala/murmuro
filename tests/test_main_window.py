...
hotkey) rides ``MurmurApp.reload_config`` in-process. See #38.
"""
...
def test_hotkey_change_emits_config_saved_without_relaunch(qapp):
    """A hotkey change is applied in-process by MurmurApp.reload_config
    via the config_saved signal. _persist_changes must emit it and must
    not invoke any restart machinery."""
    saved: list[config_mod.Config] = []
    emitted: list[config_mod.Config] = []
    win = MainWindow(_make_cfg(), save_config=saved.append)
    win.config_saved.connect(emitted.append)
